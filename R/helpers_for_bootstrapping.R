...
compare_boot_nets=function(bootnets1, bootnets2, network_metrics) { 
  ...
  for (i in 1:length(orig.diff)) {
    ...
    ans[i]=1-stats::pt(abs(t.stat),df=length(boot1.stats[i,])-1)
  }
...
p_value_matrix <- function(network, size_subnet, n.iter = 10, network_metrics, n_versions){
  ...
  for(t in 1:n.iter){
    ...
    g1_matrix <- igraph::as_adjacency_matrix(g_sub[[1]], type = "both", edges = FALSE, sparse = FALSE, attr = "weight")
    g2_matrix <- igraph::as_adjacency_matrix(g_sub[[2]], type = "both", edges = FALSE, sparse = FALSE, attr = "weight")
...
CI_matrix <- function(network, size_subnet, n_versions, n.iter = 10, network_metrics){
  ...
  length_CI <- data.frame(temp = numeric(0))
  for(i in 1:length(network_metrics)){
    length_CI[[network_metrics[i]]] <- numeric(0)
  }
  length_CI <- length_CI[,-1]
  ...
  for(t in 1:n.iter){
    ...
    g_matrix <- igraph::as_adjacency_matrix(g_sub, type = "both", edges = FALSE, sparse = FALSE, attr = "weight")
    ...
    length_CI[t,] <- CI_width_nets(g_boot, network_metrics)
  }
